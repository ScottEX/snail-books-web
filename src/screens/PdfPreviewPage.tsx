...
interface BatchData {
  ...
  total: number;
  items: { product_name: string; spec: string; unit_price: number; quantity: number; subtotal: number }[];
  operator?: string;
}
...
.pv-seal-round{width:72px;height:72px;border-radius:50%;border:2.5px solid rgba(125,35,41,.25);display:flex;align-items:center;justify-content:center;flex-direction:column}
.pv-seal-text{font-size:15px;font-weight:700;color:rgba(125,35,41,.35);letter-spacing:2px;line-height:1}
.pv-seal-sub{font-size:6.5px;font-weight:600;color:rgba(125,35,41,.2);letter-spacing:1px;margin-top:2px}
...
function fmtMoney(n: number): string { return '¥' + n.toLocaleString('zh-CN', { minimumFractionDigits: 2 }); }
...
function buildDocHTML(b: BatchData): string {
  ...
  const pm = b.payment_method === 'cash' ? '现金' : (b.payment_method === 'wechat' ? '微信' : (b.payment_method || '微信'));
  const d = (() => { try { const x = new Date(b.date + 'T00:00:00'); return `${x.getFullYear()}年${x.getMonth()+1}月${x.getDate()}日`; } catch { return b.date; } })();
...
<div><span class="lbl">日期</span> <span class="val">${d}</span></div>
<div><span class="lbl">支付</span> <span class="val">${pm}</span></div>
...
<tbody>${itemsRows}<tr class="pv-total-row"><td colspan="4" style="text-align:right;padding-right:20px;">合计（人民币）</td><td>${fmtMoney(b.total)}</td></tr></tbody>
</table>
...
export default function PdfPreviewPage({ batchId, batchNumber, onBack }: Props) {
  ...
  const ziRef = useRef<HTMLDivElement|null>(null);
  ...
  useEffect(() => { let ok = true;
    (async () => { try { const d = await api.getProcurementBatchDetail(batchId); if(ok) setBatch(d as any); } catch {} finally { if(ok) setLoading(false); } })();
    return () => { ok = false; };
  }, [batchId]);
  ...
  // ── initZoom ──
  useEffect(() => {
    if (!batch || !vpRef.current || !sheetRef.current) return;
    const t = setTimeout(() => {
      const vp = vpRef.current!;
      const paper = sheetRef.current!.querySelector('.pv-paper') as HTMLElement;
      if (!paper) return;
      const fit = Math.min(1, (vp.clientWidth - 24) / (paper.offsetWidth + 24));
  ...
  const applyTransform = useCallback((animated: boolean) => {
    ...
    sh.style.transition = animated ? 'transform .25s cubic-bezier(.4,0,.2,1)' : 'none';
    sh.style.transform = `translate(calc(-50% + ${g.tx}px), ${g.ty}px) scale(${g.scale})`;
    if (animated) setTimeout(() => { if (sh) sh.style.transition = 'none'; }, 260);
  }, []);

  const clamp = useCallback(() => {
    const vp = vpRef.current, sh = sheetRef.current;
    if (!vp || !sh) return;
    const paper = sh.querySelector('.pv-paper') as HTMLElement; if (!paper) return;
    const g = gRef.current;
    const dw = (paper.offsetWidth + 24) * g.scale, dh = (paper.offsetHeight + 24) * g.scale;
    const mx = Math.max(0, (dw - vp.clientWidth) / 2), my = Math.max(0, (dh - vp.clientHeight) / 2 + 20);
    g.tx = Math.max(-mx, Math.min(mx, g.tx)); g.ty = Math.max(-20, Math.min(my, g.ty));
  }, []);

  const flashZoom = useCallback(() => {
    setZoomVis(true); clearTimeout(ziTimer.current); ziTimer.current = setTimeout(() => setZoomVis(false), 1500);
  }, []);

  const updateZoom = useCallback((ns: number) => {
    gRef.current.scale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, ns));
    clamp(); applyTransform(true);
  }, [clamp, applyTransform]);

  // ── 手势事件（参考 pdf-viewer_1.html 的写法）──
  useEffect(() => {
    if (!batch) return;
    ...
    const onWh = (e: WheelEvent) => {
      ...
      clamp(); applyTransform(false); flashZoom();
    };
    ...
    const onTS = (e: TouchEvent) => { e.preventDefault();
      if (e.touches.length === 1) {
        const now = Date.now();
        if (now - lastTapRef.current < 300) {
          const ns = gRef.current.scale > 1.1 ? 1 : 2;
          gRef.current.scale = ns; gRef.current.tx = 0; gRef.current.ty = 0;
          clamp(); applyTransform(true); flashZoom(); lastTapRef.current = 0; return;
        }
    ...
    const onTM = (e: TouchEvent) => { e.preventDefault();
      if (dragRef.current.active && e.touches.length === 1) {
        ...
        gRef.current.scale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, pinchRef.current.scale * (dist(e.touches) / pinchRef.current.dist)));
        clamp(); applyTransform(false); flashZoom();
      }
    };
    const onTE = (e: TouchEvent) => {
      if (e.touches.length === 0) { dragRef.current.active = false; vp.classList.remove('grabbing'); }
  ...
  }, [batch, clamp, applyTransform, flashZoom]);

  const docHtml = useMemo(() => batch ? buildDocHTML(batch) : '', [batch]);
  const showToast = useCallback((icon:string, text:string) => { setToastMsg({icon,text}); clearTimeout(toastTimer.current); toastTimer.current = setTimeout(()=>setToastMsg(null), 2200); }, []);
  const doDownload = useCallback(() => { const a=document.createElement('a'); a.href=pdfUrl; a.download=`procurement_${batchId}.pdf`; document.body.appendChild(a); a.click(); document.body.removeChild(a); }, [pdfUrl,batchId]);
  const doCopyLink = useCallback(() => { navigator.clipboard?.writeText(window.location.href).catch(()=>{}); showToast('🔗','链接已复制'); }, [showToast]);

  return (
    <View style={st.container}>
      {createPortal(<div style={{position:'fixed',inset:0,zIndex:9999}}>
        <style dangerouslySetInnerHTML={{__html:CSS}}/>
        {loading && <div className="pv-loading"><div className="pv-spinner"/></div>}
        {batch && <>
          <div className="pv-nav"><div className="pv-nav-l">
            ...
            <div><div className="pv-title">{title}</div><div className="pv-sub">NO.2026-{String(batch.batch_number).padStart(4,'0')}</div></div>
          </div></div>
          <div className="pv-pill">第 1 页 / 共 1 页</div>
          <div className={`pv-zi${zoomVis?' on':''}`} ref={ziRef}>{Math.round(gRef.current.scale*100)}%</div>
          <div className="pv-vp">
          ...
          <div className="pv-zoom-strip">
            <div className="pv-zoom-btn" onClick={()=>updateZoom(gRef.current.scale+0.25)}><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
            <div className="pv-zoom-btn" onClick={()=>{gRef.current.scale=1;gRef.current.tx=0;gRef.current.ty=0;clamp();applyTransform(true);flashZoom();}}><svg viewBox="0 0 24 24"><path d="M3.5 3.5l4 4M20.5 3.5l-4 4M20.5 20.5l-4-4M3.5 20.5l4-4"/></svg></div>
            <div className="pv-zoom-btn" onClick={()=>updateZoom(gRef.current.scale-0.25)}><svg viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
          </div>
          <div className="pv-tb">
          ...
          <div className={`pv-sh-overlay${shareOpen?' open':''}`} onClick={()=>setShareOpen(false)}>
            <div className="pv-sh" onClick={e=>e.stopPropagation()}>
              <div className="pv-sh-handle"/><div className="pv-sh-title">分享进货单</div>
              ...
              ].map(([l,cl,p])=><div key={l} className="pv-sh-act" onClick={()=>{setShareOpen(false);showToast('📤',`已分享至 ${l}`);}}><div className="pv-sh-icon" style={{background:cl}}><svg viewBox="0 0 24 24"><path d={p}/></svg></div><span>{l}</span></div>)}</div>
